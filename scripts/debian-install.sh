...
# GamePlan Debian Installation Script
# This script automates the installation of GamePlan on Debian 11/12
...
check_debian_version() {
    ...
    if [[ -f /etc/os-release ]]; then
        . /etc/os-release
        if [[ "$ID" != "debian" ]]; then
            error "This script is designed for Debian. Detected OS: $ID"
            exit 1
        fi
        
        case "$VERSION_ID" in
            "12"|"11")
                log "Debian $VERSION_ID ($VERSION_CODENAME) detected - supported version"
                ;;
            "10")
                warn "Debian 10 (Buster) detected - limited support"
                ;;
            *)
                warn "Debian $VERSION_ID detected - not tested, proceeding anyway"
                ;;
        esac
    else
...
verify_docker() {
    ...
    # Test Docker functionality with newgrp
    if newgrp docker <<< 'docker run --rm hello-world' &>/dev/null; then
        log "Docker test successful"
    else
        warn "Docker test failed - will require logout/login for full functionality"
    fi
}
...
deploy_gameplan() {
    ...
    # Use newgrp to ensure docker group membership
    newgrp docker <<EOF
        # Start services
        docker compose up -d
...
verify_deployment() {
    ...
    # Check container status using newgrp
    if newgrp docker <<< 'docker compose ps' | grep -q "Up"; then
        log "Containers are running"
    else
        error "Some containers are not running"
        newgrp docker <<< 'docker compose ps'
        exit 1
    fi
...
${BLUE}Next Steps:${NC}
1. ${YELLOW}IMPORTANT: Logout and login again to use Docker without sudo${NC}
2. Update admin email in .env file: nano .env
3. Configure domain name (optional)
4. Set up SSL certificate (see DEBIAN_DEPLOYMENT.md)
5. Set up reverse proxy (see DEBIAN_DEPLOYMENT.md)
...
- You may see Docker permission warnings until you logout/login
- STEAM_API_KEY warning is normal - Steam integration works without it
- Docker Compose version warnings are harmless
...
# Fix Docker permissions post-install
fix_docker_permissions() {
    log "Setting up Docker permissions..."
    
    cat << EOF

${YELLOW}IMPORTANT: Docker Group Membership${NC}

Your user has been added to the docker group, but you need to logout and login again
for the changes to take effect. Until then, you may see permission errors.

${BLUE}To apply changes immediately:${NC}
1. Logout of your current session
2. Login again
3. Verify with: docker ps

${BLUE}Or run this command and then logout/login:${NC}
newgrp docker
...
main() {
    log "Starting GamePlan Debian installation..."
